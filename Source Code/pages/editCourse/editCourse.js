...
Page({
  ...
  onLoad: function (options) {
    var CourseOrder = Number(options.CourseOrder)
    var CourseList = wx.getStorageSync("COURSELIST")
    CurrentCourse = CourseList[CourseOrder - 1]
    var OtherExist = CurrentCourse.OtherExist
    AddOther = OtherExist
    Info.Name = CurrentCourse.Name
    Info.Number = CurrentCourse.Number
    Info.Instructor = CurrentCourse.Instructor
    Info.Location = CurrentCourse.Location
    this.setData({
      NameVal: CurrentCourse.Name,
      NumberVal: CurrentCourse.Number,
      InstructorVal: CurrentCourse.Instructor,
      LocationVal: CurrentCourse.Location
    })

    /* Other */

    if (OtherExist) {
      Info.OtherType = CurrentCourse.Other[0]
      Info.OtherLocation = CurrentCourse.Other[1]
      OSTH = CurrentCourse.Other[2].Start
      OETH = CurrentCourse.Other[2].End
      var DaySelected = CurrentCourse.Other[2].Day
      var DayLen = DaySelected.length
      for (var i = 0; i < DayLen; i++){
        var CurrentDay = DaySelected[i]
        switch(CurrentDay){
          case "M": this.setData({OtherDayMSelected: true}); break;
          case "T": this.setData({OtherDayTSelected: true}); break;
          case "W": this.setData({OtherDayWSelected: true}); break;
          case "Th": this.setData({OtherDayThSelected: true}); break;
          case "F": this.setData({OtherDayFSelected: true}); break;
          default: break;
        }
      }
      var OSTString = common.TimeString(OSTH)
      var OETString = common.TimeString(OETH)
      this.setData({
        OtherTypeInput: CurrentCourse.Other[0],
        OtherLocationInput: CurrentCourse.Other[1],
        Adding: true,
        OtherTimeStart: OSTString,
        OtherTimeEnd: OETString,
      })
    }

    /* Time */

    var TimeList = CurrentCourse.Time
    var TimeLen = TimeList.length
    currentTimes = TimeLen
    for (var i = 0; i < TimeLen; i++){
      STH[i] = TimeList[i].Start
      ETH[i] = TimeList[i].End
      var CurrentTime = TimeList[i]
      var CurrentDay = CurrentTime.Day
      var CurrentSTH = CurrentTime.Start
      var CurrentETH = CurrentTime.End
      var CurrentSTString = common.TimeString(CurrentSTH)
      var CurrentETString = common.TimeString(CurrentETH)
      this.data.timeList.push(i + 1)
      this.data.timeStart.push(CurrentSTString)
      this.data.timeEnd.push(CurrentETString)
      ...
      for (var j = 0; j < DayLen; j++){
        var CDay = CurrentDay[j]
        switch (CDay) {
          case "M": DayM = true; break;
          case "T": DayT = true; break;
          case "W": DayW = true; break;
          case "Th": DayTh = true; break;
          case "F": DayF = true; break;
          default: break;
        }
      }
      this.data.DayMSelected.push(DayM)
      this.data.DayTSelected.push(DayT)
      this.data.DayWSelected.push(DayW)
      this.data.DayThSelected.push(DayTh)
      this.data.DayFSelected.push(DayF)
      this.setData({
  ...
  SubmitBtnClick: function () {
    ...
    if (passOK) {
      var CourseList = wx.getStorageSync("COURSELIST")
      var updatedCourse = common.returnCourse(passVal)
      var index = CurrentCourse.Order - 1
      common.needUpdateList = true
      CourseList[index] = updatedCourse
      wx.setStorageSync("COURSELIST", CourseList)
      wx.reLaunch({
        url: '../settings/settings?Edited=true',
      })
